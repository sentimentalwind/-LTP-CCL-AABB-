...
col = ('序号','出处','清洗完成的句子','aabb','词性','依存句法关系1','关系2','关系3','关系4','关系5','关系6')
for i in range(0,11):
...
with open("corpus_pattern_AABB.txt","r",errors='ignore')as f:#以只读方式打开文件
  for line in f.readlines():#对每一行进行循环操作
    ...
    if inc!=-1:
     ...
     sheet.write(ex_line,3,aabb)
     sheet.write(ex_line,4,_863dict[pos[0][inc]])
     list_dep=[]
     for temp in dep[0]:
       if temp[0]==inc+1:
         list_dep.append(temp[2])
       if temp[1]==inc+1:
         list_dep.append(temp[2])
     for i in range(0,len(list_dep)):
       sheet.write(ex_line,5+i,_syn_rl_dict[list_dep[i]])
